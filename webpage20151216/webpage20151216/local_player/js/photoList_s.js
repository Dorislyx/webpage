...
function keyRight()
{
	...
	var row = parseInt((fileIndex%ViewTotal)/3)%2;
	if (isMiniMenu) {
		miniMenuNext();
	}
	else if (isAddPlayList && !isEditPlayList && (row>=1 ||(fileIndex >= (fileTotal-1)))) {
		enablePlayList();
	}
	else if (isAddOtherList && !isEditOtherList && (row>=1 ||(fileIndex >= (fileTotal-1)))) {
		enablePlayList();
	}
	else {
		channelRight();
...
function getFileList()
{
	if (httpRequest.readyState==4 && httpRequest.status==200) {
		...
		if (getStatus==0) {
			//hideFileList();
			document.getElementById('messageText').innerHTML = photo_OPERATE_INFO[0]+photo_OPERATE_INFO[1];
...
function showFileList()
{
	...
	var showPageUrl = (pageCurrent>0)? ('url(style1/pageUp_S.png)') : ('url(style1/pageUp_N.png)');
	document.getElementById('pageUpStyle').style.backgroundImage = showPageUrl;
	showPageUrl = (pageCurrent<pageTotal)? ('url(style1/pageDn_S.png)') : ('url(style1/pageDn_N.png)');
	document.getElementById('pageDnStyle').style.backgroundImage = showPageUrl;
...
function showFileInfo()
{
	...
	showStr += photo_FILE_INFO[0]+typeStr+'&nbsp;&nbsp;|&nbsp;&nbsp;';
	if (fileType[pos]!=0) {
		showStr += photo_FILE_INFO[1]+sizeStr+'&nbsp;&nbsp;|&nbsp;&nbsp;';
	}
	showStr += photo_FILE_INFO[2]+timeStr[0]+'&nbsp;&nbsp;|&nbsp;&nbsp;';
...
{
	document.getElementById('styleList').style.height = "0px";
	document.getElementById('pageUpStyle').style.visibility = 'hidden';
	document.getElementById('pageDnStyle').style.visibility = 'hidden';
	hideFileInfo();
	return;
}
...
function channelLeft()
{
	if (fileIndex<=0 || SETUP_WebStyle==0) {
		return;
	}
	var pos = fileIndex%ViewTotal;
	if (pos<3) {
...
function gotoFullScreen()
{
	...
	isKeyClock = true;
	//document.getElementById('fullWindows').innerHTML = photo_LOAD_INFO[0];
...
function showBgList()
{
	...
	var tempPageNumber = photo_PAGE_NUMBER[0]+(currentPage+1)+photo_PAGE_NUMBER[1]+(totalPage+1)+photo_PAGE_NUMBER[2];
...
function getMusicList()
{
	if (httpRequest.readyState==4 && httpRequest.status==200) {
		...
		isMiniMenu = true;
		miniMenuShow();		
		document.getElementById('divMenuFocus').style.backgroundImage = 'url(style1/skin'+SETUP_SkinIndex+'/menuIconFocus.png)';
		document.getElementById('miniMenuBg').style.backgroundImage = 'url(style1/skin'+SETUP_SkinIndex+'/miniMenu4.png)';
...
/* Description: 压缩九宫格显示为六宫格                               */
/* Parameters:                                                       */
/* Author&Date: lixudong  2010-03-11                                 */
/*********************************************************************/
function compressList()
{
	document.getElementById('rightDiv').style.visibility = 'hidden';
