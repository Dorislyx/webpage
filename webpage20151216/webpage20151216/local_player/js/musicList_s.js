...
function keyRight()
{
	...
	else if (isAddPlayList && !isEditPlayList && ((fileTotal-fileIndex)<=3||pos>=ViewTotal-3)) {
		enablePlayList();
	}
	else if (isAddOtherList && !isEditOtherList && ((fileTotal-fileIndex)<=3||pos>=ViewTotal-3)) {
		enablePlayList();
	}
	else {
		channelRight();
...
function keyChannelUp()
{
	if (isMiniMenu || isAddPlayList || playStatus==0) {
		return;
	}
	showKeyInfoChannel(1);
	playNext();
	return;
}
...
function keyChannelDown()
{
	...
	showKeyInfoChannel(0);
	playPrevious();
	return;
}

/*********************************************************************/
/* Function: showKeyInfoVol                                          */
/* Description: 按键指示处理(音量)                                   */
/* Parameters:                                                       */
/* Author&Date: lixudong  2010-03-30                                 */
/*********************************************************************/
var showKeyInfoTime = 400; //播放器按键显示时间
var timerID_KeyInfo; //播放器按键显示定时器
function showKeyInfoVol()
{
	hideKeyInfo();
	if (volumeOperate==1) {
		document.getElementById('playerVolUp').style.visibility = 'visible';
	}
	else if (volumeOperate==2) {
		document.getElementById('playerVolDn').style.visibility = 'visible';
	}
	else {
		return;
	}
	timerID_KeyInfo = setTimeout('hideKeyInfo()', showKeyInfoTime);
	return;
}

/*********************************************************************/
/* Function: showKeyInfoPlayer                                       */
/* Description: 按键指示处理(播放暂停)                               */
/* Parameters: flag播放暂停状态                                      */
/* Author&Date: lixudong  2010-03-30                                 */
/*********************************************************************/
function showKeyInfoPlayer(flag)
{
	hideKeyInfo();
	if (flag==1) {
		document.getElementById('playerPlay').style.visibility = 'visible';
	}
	else if (flag==2) {
		document.getElementById('playerPause').style.visibility = 'visible';
	}
	else {
		return;
	}
...
/*********************************************************************/
/* Function: showKeyInfoChannel                                      */
/* Description: 按键指示处理(上一曲下一曲)                           */
/* Parameters: flag上一曲下一曲状态                                  */
/* Author&Date: lixudong  2010-03-30                                 */
/*********************************************************************/
function showKeyInfoChannel(flag)
{
	hideKeyInfo();
	if (flag==0) {
		document.getElementById('playerLast').style.visibility = 'visible';
	}
	else if (flag==1) {
		document.getElementById('playerNext').style.visibility = 'visible';
	}
	else {
		return;
	}
...
function hideKeyInfo()
{
	clearTimeout(timerID_KeyInfo);
	document.getElementById('playerPlay').style.visibility = 'hidden';
	document.getElementById('playerPause').style.visibility = 'hidden';
	document.getElementById('playerLast').style.visibility = 'hidden';
	document.getElementById('playerNext').style.visibility = 'hidden';
	document.getElementById('playerVolUp').style.visibility = 'hidden';
	document.getElementById('playerVolDn').style.visibility = 'hidden';
	return;
}
///////////////////////////////////////////////////////////////////////按键处理部分结束
...
{
	//背景图片
	var tempURL = 'url(style1/skin'+SETUP_SkinIndex+'/mainBg1.jpg)';
	document.getElementById('styleList').style.backgroundImage = tempURL;
	//添加列表背景图片
	tempURL = 'url(style1/skin'+SETUP_SkinIndex+'/addList.png)';
	document.getElementById('addList').style.backgroundImage = tempURL;
	tempURL = 'url(style1/skin'+SETUP_SkinIndex+'/addListFocus.png)';
	document.getElementById('addListFocus').style.backgroundImage = tempURL;
	//MiniMenu背景图片
	tempURL = 'url(style1/skin'+SETUP_SkinIndex+'/miniMenu5.png)';
...
function getFileList()
{
	if (httpRequest.readyState==4 && httpRequest.status==200) {
		...
		if (getStatus==0) {
			//hideFileList();
			document.getElementById('messageText').innerHTML = music_OPERATE_INFO[0]+music_OPERATE_INFO[1];
			document.getElementById('messageText').style.backgroundImage = 'url(style1/skin'+SETUP_SkinIndex+'/miniMenu3.png)';
...
{
	var pos = fileIndex%ViewTotal;
	filePicture[pos] = 'style1/fileType'+fileType[pos]+'.png';
...
function showFileInfo()
{
	...
	var tempLeft = (ViewTotal==9)? (row*259+450) : (row*175+398);
	...
	showStr += music_FILE_INFO[0]+typeStr+'&nbsp;&nbsp;|&nbsp;&nbsp;';
	if (fileType[pos]!=0) {
		showStr += music_FILE_INFO[1]+sizeStr+'&nbsp;&nbsp;|&nbsp;&nbsp;';
	}
	showStr += music_FILE_INFO[2]+timeStr[0]+'&nbsp;&nbsp;|&nbsp;&nbsp;';
...
function getPlayList()
{
	if (httpRequest.readyState==4 && httpRequest.status==200) {
		...
		playStatus = 1;
		document.getElementById('playerBg').style.visibility = 'visible';
		compressList();
...
function showPlayInfo()
{
	if (httpRequest.readyState==4 && httpRequest.status==200) {
		...
		if (wordTotal==0 || wordShowMode==0) {
			...
			wordText = new Array();
			showPlayInfoDiv();
			document.getElementById('playerWord1').innerHTML = '';
			document.getElementById('playerWord2').innerHTML = '';
			//全屏状态执行
			...
			//显示歌词信息
			showWordInfoDiv();
			document.getElementById('playerFile').innerHTML = '';
			//全屏状态执行
...
function showWordInfoDiv()
{
	...
	document.getElementById('playerWord1').innerHTML = wordText[wordIndex];
	document.getElementById('playerWord2').innerHTML = showStr;
	//全屏状态执行
...
function showPlayInfoDiv()
{
	...
	showStr += music_FILE_INFO[2]+timeStr[0];
	showStr += ' '+timeStr[1]+'<br>';
	showStr += music_FILE_INFO[4]+nameStr;
	document.getElementById('playerFile').innerHTML = showStr;
...
function showPlayStatus()
{
	var tempStatus = (playStatus==2)? 'visible' : 'hidden';
	document.getElementById('playerPauseBar').style.visibility = tempStatus;
	showKeyInfoPlayer(playStatus);
	//全屏状态执行
	var imgURL = '';
	if (playStatus==1) {
	...
	document.getElementById('musicPlayStatusFull').src = imgURL;
	return;
}
...
function playEnd()
{
	...
	if (playIndex>=playTotal) {
		if (cycleType==1 || cycleType==2) {
			...
			playStatus = 0;
			document.getElementById('playerBg').style.visibility = 'hidden';
...
function screenSaverPlay()
{
	...
	var tempPos_x = parseInt(Math.random()*1080);
	var tempPos_y = parseInt(Math.random()*540);
...
function screenSaverExit()
{
	if (httpRequest.readyState==4 && httpRequest.status==200) {
		...
		hideOrVisibleDiv(1);
		if (isAddPlayList || isAddOtherList || playStatus==0 ) {
			document.getElementById('playerBg').style.visibility = 'hidden';
			document.getElementById('fullPlayInfo').style.visibility = 'hidden';
		}
		else {
...
function showProcessInfo()
{
	...
	//列表状态状态
	var len = (totalTime==0||currentTime==0)? 1 : parseInt(100*currentTime/totalTime);
	len = (len>100)? 100 : len;
	len = (len<1)? 1 : len;
	document.getElementById('playerBar').style.width = len+"px";
	document.getElementById('playerText').innerHTML = timeStr;
	//全屏播放状态
...
/* Description: 显示音量条                                           */
/* Parameters:                                                       */
/* Author&Date: lixudong  2010-04-16                                 */
/*********************************************************************/
function showVolume()
{
	var tempIcon = (isMuteStatus==1)? 'url(style1/speakerOff.png)': 'url(style1/speakerOn.png)';
	document.getElementById('volumeIcon').style.backgroundImage = tempIcon;
	var tempBar = (isMuteStatus==1)? 'url(style1/barGray.png)': 'url(style1/barGreen.png)';
	document.getElementById('volumeBar').style.backgroundImage = tempBar;
	...
	if (!isFullScreen) {
		...
		var tempStatus = (len==1)? 'hidden' : 'visible';
		document.getElementById('volumeBar').style.visibility = tempStatus;
		document.getElementById('volumeText').innerHTML = showStr;
		document.getElementById('volumePart').style.visibility = 'visible';
	}
	//全屏播放状态
	else if (isFullScreen) {
		var lenFull = (457*volumeLevel)/100;
		lenFull = (lenFull<=0)? 1 : lenFull;
		lenFull = (lenFull>=457)? 457 : lenFull;
...
function hideVolume()
{
	...
	document.getElementById('volumePart').style.visibility = 'hidden';
	document.getElementById('musicVolume').style.visibility = 'hidden';
	return;
}
///////////////////////////////////////////////////////////////////////音量状态显示部分结束
...
{
	//背景音乐列表提示信息
	//document.getElementById('addList').style.backgroundImage = 'url(style1/skin'+SETUP_SkinIndex+'/addList.png)';
	//document.getElementById('addListFocus').style.backgroundImage = 'url(style1/skin'+SETUP_SkinIndex+'/addListFocus.png)';
	if (isAddPlayList) {
		document.getElementById('addListTitle').innerHTML = music_OPERATE_INFO[2];
		document.getElementById('addListInfo0').innerHTML = music_LIST_MESSAGE[0];
		document.getElementById('addListInfo1').innerHTML = music_LIST_MESSAGE[1];
	}
	else if (isAddOtherList) {
		document.getElementById('addListTitle').innerHTML = photo_OPERATE_INFO[2];
		document.getElementById('addListInfo0').innerHTML = photo_LIST_MESSAGE[0];
		document.getElementById('addListInfo1').innerHTML = photo_LIST_MESSAGE[1];
	}
	return;
}
...
function playListEnd()
{
	...
	document.getElementById('styleFocus').style.visibility = 'visible';
	document.getElementById('addListFocus').style.visibility = 'hidden';
...
function getBgList()
{
	if (httpRequest.readyState==4 && httpRequest.status==200) {
		...
		bgListIndex = 0;
		document.getElementById('addListFocus').style.top = '62px';
		showBgList();
	}
	return;
}
...
function showBgList()
{
	...
	//document.getElementById('addListPageUp').style.visibility = showStatus;
	//showStatus = (currentPage<totalPage)? 'visible' : 'hidden';
	//document.getElementById('addListPageDn').style.visibility = showStatus;
	var tempPageNumber = music_PAGE_NUMBER[0]+(currentPage+1)+music_PAGE_NUMBER[1]+(totalPage+1)+music_PAGE_NUMBER[2];
	document.getElementById('addListPageText').innerHTML = tempPageNumber;
	//显示文件列表
	for (var i=0; i<8; i++) {
		var pos = i+currentPage*8;
		var showStr = (pos<bgListTotal) ? bgListShowName[pos] : '';
		document.getElementById('addList'+i).innerHTML = showStr;
	}
	showBgListInfo();
	return;
}
...
function showBgListInfo()
{
	...
	document.getElementById('addListFocus').style.top = (55+42*pos)+"px";
	return;
}
...
function enablePlayList()
{
	...
	if (isAddPlayList) {
		isEditPlayList = true;
		document.getElementById('addListInfo0').innerHTML = music_LIST_MESSAGE[2];
	}
	else if (isAddOtherList) {
		isEditOtherList = true;
		document.getElementById('addListInfo0').innerHTML = photo_LIST_MESSAGE[2];
	}
	document.getElementById('styleFocus').style.visibility = 'hidden';
	document.getElementById('addListFocus').style.visibility = 'visible';
	return;
}
...
function miniMenuShow()
{
	...
	//添加背景音乐图标
	var tempStatus = isAddOtherList ? 33 : 3;
...
{
	document.getElementById('rightDiv').style.visibility = 'hidden';
	document.getElementById('leftDiv').style.left = (142-40)+"px";
	document.getElementById('lineDiv').style.left = (369-40)+"px";
	document.getElementById('middleDiv').style.left = (440-12-40)+"px";
	document.getElementById('style3').style.left = (285-84)+"px";
	document.getElementById('style4').style.left = (285-84)+"px";
	document.getElementById('style5').style.left = (285-84)+"px";
	document.getElementById('styleText3').style.left = (275-84)+"px";
	document.getElementById('styleText4').style.left = (275-84)+"px";
	document.getElementById('styleText5').style.left = (275-84)+"px";
	var tempLeft = parseInt((fileIndex%6)/3)*175+398;
	document.getElementById('styleFocus').style.left = tempLeft+"px";
	ViewTotal = 6;
	getFileListRequest();
	return;
...
/*********************************************************************/
/* Function: uncompressList                                          */
/* Description: 从六宫格扩展为九宫格                                 */
/* Parameters:                                                       */
/* Author&Date: lixudong  2010-03-28                                 */
/*********************************************************************/
function uncompressList()
{
	...
	document.getElementById('leftDiv').style.left = "142px";
	document.getElementById('lineDiv').style.left = "369px";
	document.getElementById('middleDiv').style.left = "440px";
	document.getElementById('rightDiv').style.visibility = 'visible';
	document.getElementById('style3').style.left = "285px";
	document.getElementById('style4').style.left = "285px";
	document.getElementById('style5').style.left = "285px";
	document.getElementById('styleText3').style.left = "275px";
	document.getElementById('styleText4').style.left = "275px";
	document.getElementById('styleText5').style.left = "275px";
	var tempLeft = parseInt((fileIndex%9)/3)*259+450;
	document.getElementById('styleFocus').style.left = tempLeft+"px";
	ViewTotal = 9;
	getFileListRequest();
	return;
